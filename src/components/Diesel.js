...
function Diesel() {
  ...
  const [price, setprice] = useState(1.89);
   ...
   useEffect(()=>{
    setprice((litre * dieselprice).toFixed(2)); 
   }, [litre])
   ...
   const handleClick = () =>{
    ...
    stop();
    setblock(true)
    localStorage.setItem("fueltype", "Diesel");
    localStorage.setItem("litre", litre);
    localStorage.setItem("pprice", dieselprice);
  ...
  return (
    <div className="App">
         <h1 style={{color:"#01295c", fontWeight:"Bold", fontSize: 50, marginTop:40}}>Diesel</h1> 
